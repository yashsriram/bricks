...
pub trait IntersectAware {
    fn does_intersect(&self, p1: Point2<f32>, p2: Point2<f32>) -> bool;
    fn does_contain(&self, point: Point2<f32>, margin: f32) -> bool;
}

pub mod obstacle {
    use super::IntersectAware;
    ...
    impl CircleObstacle {
        pub fn new(center: Point2<f32>, radius: f32) -> Self {
            assert!(radius > 0.0);
            CircleObstacle { center, radius }
        }
    }

    impl IntersectAware for CircleObstacle {
        fn does_intersect(&self, p1: Point2<f32>, p2: Point2<f32>) -> bool {
            let pb_pa = p2 - p1;
    ...
    impl LineSegmentObstacle {
        pub fn new(e1: Point2<f32>, e2: Point2<f32>) -> Self {
            ...
            LineSegmentObstacle { e1, e2 }
        }
    }

    impl IntersectAware for LineSegmentObstacle {
        fn does_intersect(&self, p1: Point2<f32>, p2: Point2<f32>) -> bool {
        ...
        fn does_contain(&self, point: Point2<f32>, margin: f32) -> bool {
            ...
            let min_dist_from_line_segement = (point - nearest_point).norm();
...
pub mod pos_conf_space {
    ...
    pub struct PosConfSpace {
        pub obstacles: Vec<Box<dyn IntersectAware>>,
    }

    impl PosConfSpace {
        pub fn does_intersect(&self, p1: Point2<f32>, p2: Point2<f32>) -> bool {
            self.obstacles.iter().any(|obs| obs.does_intersect(p1, p2))
        }

        pub fn does_contain(&self, point: Point2<f32>, margin: f32) -> bool {
            self.obstacles
                .iter()
                .any(|obs| obs.does_contain(point, margin))
        }
    }
}
