pub mod fringe {
    ...
    pub trait VertexSearchState<S: StateSpace>:
        Debug + Clone + CostBasedPriority + Propagate<S>
    {
    }
    ...
    impl<S: StateSpace, F: VertexSearchState<S>> FringeBasedSearch<S, F> {
        ...
        // pub fn path_to(&self, finish_idx: usize) -> Vec<usize> {
        //     let mut node = finish_idx;
        //     let mut path = vec![];
        //     while let Some(parent) = self.vertex_search_states[&node].parent {
        //         path.push(parent);
        //         node = parent;
        //     }
