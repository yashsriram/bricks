use bevy::prelude::*;
use bricks::plan::graph::prm::PRM;
use bricks::plan::graph::search::tree::propagations::*;
use bricks::plan::graph::search::tree::TreeSearch;
use bricks::plan::spaces::CuboidSpace;
use bricks::*;

fn main() {
    App::build()
        .insert_resource(Msaa { samples: 1 })
        .insert_resource(ClearColor(Color::rgb(0.0, 0.0, 0.0)))
        .add_plugins(BasePlugins)
        .add_startup_system(setup.system())
        .run();
}

fn setup(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>) {
    let space = CuboidSpace {
        size: Vec3::new(12.0, 10.0, 5.0),
    };
    let mut prm = PRM::with_num_samples(space, 2000, 1.0);
    let idxes = prm.add(
        vec![Vec3::new(0.3, 0.7, 0.5), Vec3::new(9.5, 7.3, 4.0)],
        1.0,
    );

    use bevy::render::pipeline::RenderPipeline;
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(
            TreeSearch::<CuboidSpace, DFSLike>::try_search(&prm.graph, idxes[0], idxes[1]).into(),
        ),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        transform: Transform::from_xyz(14.0, 0.0, 0.0),
        ..Default::default()
    });
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(
            TreeSearch::<CuboidSpace, BFSLike>::try_search(&prm.graph, idxes[0], idxes[1]).into(),
        ),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        transform: Transform::from_xyz(28.0, 0.0, 0.0),
        ..Default::default()
    });
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(
            TreeSearch::<CuboidSpace, UCSLike>::try_search(&prm.graph, idxes[0], idxes[1]).into(),
        ),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        transform: Transform::from_xyz(40.0, 0.0, 0.0),
        ..Default::default()
    });
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(
            TreeSearch::<CuboidSpace, AStarLike>::try_search(&prm.graph, idxes[0], idxes[1]).into(),
        ),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        transform: Transform::from_xyz(54.0, 0.0, 0.0),
        ..Default::default()
    });
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(
            TreeSearch::<CuboidSpace, W2AStarLike>::try_search(&prm.graph, idxes[0], idxes[1])
                .into(),
        ),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        transform: Transform::from_xyz(68.0, 0.0, 0.0),
        ..Default::default()
    });
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(prm.state_space.into()),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        ..Default::default()
    });
    commands.spawn_bundle(MeshBundle {
        mesh: meshes.add(prm.graph.into()),
        render_pipelines: RenderPipelines::from_pipelines(vec![RenderPipeline::new(
            vis::WIREFRAME_PIPELINE_HANDLE.typed(),
        )]),
        ..Default::default()
    });
}
